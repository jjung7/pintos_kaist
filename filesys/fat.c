...
{
	for (cluster_t idx = 0; idx < fat_fs->fat_length; idx++)
	{
		if (clst == 0 && fat_fs->fat[idx] == 0)
		{
			fat_fs->fat[idx] = EOChain;
			fat_fs->last_clst = idx;
			return idx;
		}
		// New chain start
		else if (fat_fs->fat[idx] == 0)
		{
			fat_fs->fat[fat_fs->last_clst] = idx;
			fat_fs->fat[idx] = EOChain;
			fat_fs->last_clst = idx;
			return idx;
		}
